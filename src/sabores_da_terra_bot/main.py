    ...
    while True:
        try:
            ...
            answer = await client.ask(
                user_id, textwrap.dedent(msg), filters=filters.text, timeout=30
            )

        except ListenerTimeout:
            await message.reply('⏰ Tempo esgotado! Use /produtos novamente.')
            return
    ...
    **Redes Sociais**
    ...
    await message.reply(
        ...
        reply_markup=ReplyKeyboardMarkup(
            [['/produtos', '/site', '/social']], resize_keyboard=True
        ),
    )
